    ...
    return (
        <button 
            onClick={() => {
             ...
             className={`p-2 rounded-[8px] transition-all duration-300 ${copied ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-neutral-800 text-neutral-400 border border-white/[0.08] hover:text-white hover:bg-white/[0.04]'} ${className}`}
            title="Copy"
            aria-label="Copy"
        >
...
export function DriveDocumentViewer({ data }: { data: DriveDocumentData }) {
    ...
    return (
        <div className="w-full bg-neutral-950 border border-white/[0.05] rounded-[24px] overflow-hidden flex flex-col text-left font-sans mb-8 shadow-2xl">
            {/* Header */}
            <div className="p-6 border-b border-white/[0.05] bg-neutral-900 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-start gap-4">
                    <div className="p-3 bg-neutral-800 rounded-lg border border-white/[0.06]">
                        {isSheet ? (
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-1.5 text-[11px] font-mono text-neutral-500 uppercase tracking-widest">
                            ...
                            <span className="bg-white/[0.04] px-2 py-0.5 rounded-[4px] border border-white/[0.06]">{data.mimeType.split('.').pop() || data.mimeType}</span>
                        </div>
                    </div>
                </div>
                <div className="flex items-center gap-2 relative">
                    {(data.htmlContent || data.csvContent) && (
                        <button
                            ...
                            className="flex items-center gap-2 bg-white/[0.04] hover:bg-white/[0.08] px-4 py-2 rounded-full text-[12px] font-medium text-white transition-colors border border-white/[0.06] shadow-sm"
                        >
                    ...
                    {data.webViewLink && (
                        <a href={data.webViewLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white/[0.04] hover:bg-white/[0.08] px-4 py-2 rounded-full text-[12px] font-medium text-white transition-colors border border-white/[0.06] shadow-sm">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="p-2 rounded-full bg-white/[0.04] hover:bg-white/[0.08] text-white transition-colors border border-white/[0.06] shadow-sm"
                        >
                        ...
                        <AnimatePresence>
                            {showActions && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, y: 10 }}
                                    className="absolute right-0 mt-2 w-48 bg-neutral-900 border border-white/[0.08] rounded-xl shadow-lg py-2 z-20"
                                >
            ...
            <div className="relative w-full overflow-hidden bg-neutral-900" style={{ minHeight: '400px', maxHeight: '70vh' }}>
                ...
                {isSheet && data.csvContent && (
                    <div className="w-full h-full flex flex-col bg-neutral-900">
                        <div className="p-3 border-b border-white/[0.05] bg-neutral-950 flex items-center gap-3">
                            <div className="relative flex-1 max-w-sm">
                                ...
                                <input
                                    ...
                                    className="w-full bg-neutral-800 border border-white/[0.06] rounded-full pl-9 pr-4 py-1.5 text-[12px] text-white outline-none focus:border-neutral-600 transition-colors"
                                />
                                {searchQuery && (
                                    <button
                        ...
                        <div className="flex-1 overflow-auto text-[12px] font-mono relative">
                            <table className="w-full text-left border-collapse whitespace-nowrap">
                                <thead className="sticky top-0 z-10 shadow-sm">
                                    {parsedCsv.length > 0 && (
                                        <tr className="bg-neutral-900 border-b border-white/[0.05]">
                ...
                {!isContentReady && !(data.mimeType.includes('pdf') || data.mimeType.includes('image') || data.mimeType.includes('video')) && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm p-6 text-center z-10">
                         <div className="max-w-md bg-neutral-900 border border-white/[0.08] p-8 rounded-2xl shadow-2xl">
            ...
            <div className="bg-neutral-900 border-t border-white/[0.05] px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
                 ...
                 <div className="flex flex-wrap items-center gap-2">
                     <button className="flex items-center gap-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] px-3 py-1.5 rounded-lg text-[11px] font-medium text-white transition-colors shadow-sm">
            ...
            {/* Footer */}
            <div className="bg-neutral-950 border-t border-white/[0.05] px-6 py-3 flex items-center justify-between select-none">
